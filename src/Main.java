...
public class Main {
     ...
     *             如果args不为空（-e <testFile>.txt -a <answerFile>.txt），则表示将用户文件和答案文件进行比对。
     */
    public static void main(String[] args) {
        if(args.length == 0) {
            Scanner sc = new Scanner(System.in);
            System.out.print("请输入生成式子的个数：");
            int num = sc.nextInt();
            System.out.print("请输入生成式子的范围：");
            int range = sc.nextInt();
            sc.close();

            // 清空Exercises.txt,Answers.txt的内容
            MyFile.clear("test.txt");
            ...
            while (num > 0) {
                ...
                // 判断题目有无重复。为 true，代表可用
                    ...
                    if(!answer.equals("error")){
                        MyFile.write("test.txt", order + ". " + question);
